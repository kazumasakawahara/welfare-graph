...
├── raw/                    # 生ソース（既存は READ-ONLY、新規追加は承認制 — §2-1 参照）
│   ├── laws/               # 法令原文
...
1. **`raw/` 内の既存ファイルは READ-ONLY。** 既存ファイルの書き換え・削除・移動・上書きは禁止。**新規ファイルの追加は許容**するが、以下の条件をすべて満たすこと:
   - (a) 配置は人間（河原）が直接行うか、AI が河原の **明示承認**（「raw/X.pdf を置いて」「Web から取得して raw/insights/ に保存して」等）を得て行う
   - (b) 出典 URL・取得日・取得経路を `raw/.../README.md` または冒頭の HTML コメントで記録
   - (c) 著作権上配布できない一次資料（市販書籍 PDF 等）は `.gitignore` に追加してコミットしない
...
11. **投機的ページ作成の禁止。** 合言葉: **「いつか必要」は「今は不要」と同義**。新規ページ作成の必要十分条件は次の 2 つを **両方** 満たすこと:
    - (a) `raw/` に原典が配置されている（記憶・一般知識からの執筆は不可）
    - (b) 当該ページが **具体的な実務エピソード**（P-XXXX・訪問・相談・事業所対話・研修準備等）から要請されている
    - 満たさない参照は **壊れリンク** のまま残す（Obsidian の未解決リンクは「未来の需要シグナル」として機能）
    - 詳細な判断ガイドは [[90_Meta/page-creation-principles]] 参照
