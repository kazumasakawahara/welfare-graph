...
矛盾・引用・注意事項等の表現を統一するため、以下のコールアウトを採用する（[[CLAUDE#§8 Obsidian コールアウト規約]] と同期）。

| コールアウト | 用途 |
|---|---|
| `> [!warning]` | 矛盾・改正予告・重大注意 |
| `> [!quote]` | 一次資料からの引用（条文番号・ページ明記） |
| `> [!info]` | 補足・参考情報 |
| `> [!note]` | メモ・気付き |
| `> [!tip]` | 運用上のヒント |
| `> [!danger]` | 法的リスク・禁忌 |
...
```markdown
> [!warning] 令和9年改正で変更予定
> 本条文は 2027-04-01 施行予定の改正で再編される見込み。
> 最新情報は e-Gov を確認のこと。

> [!quote] 障害者総合支援法 5条1項
> この法律において「障害福祉サービス」とは、居宅介護、重度訪問介護...

> [!danger] 身体拘束の 3 要件
> 切迫性・非代替性・一時性の全てを満たさない身体拘束は虐待にあたる。
```

### 原則

- 矛盾は必ず `> [!warning]` で明示する。勝手に正誤判断しないこと（[[CLAUDE#§2 絶対遵守のガードレール]] 第 5 項）
- 引用は出典明記（条文番号・ページ・URL 等）
...
- 2026-04-20: §8 Obsidian コールアウト規約を追加
